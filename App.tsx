...
const App: React.FC = () => {
  ...
  const handleCSIImport = (data: Array<{ tag: string, unit: string, readings: SensorReading[] }>) => {
    setAssets(prevAssets => {
      ...
      data.forEach(item => {
        ...
        let assetIdx = newAssets.findIndex(a => a.name.toLowerCase() === assetName.toLowerCase());
        if (assetIdx === -1) {
          ...
          assetIdx = newAssets.length - 1;
        }
        const lastVal = item.readings[item.readings.length - 1]?.value || 0;
        const sensorUniqueId = `${pointTag}_${item.unit}`;
        const sensorIdx = newAssets[assetIdx].sensors.findIndex(s => s.id === sensorUniqueId && s.dataSource === 'periodic');
        if (sensorIdx !== -1) {
          newAssets[assetIdx].sensors[sensorIdx] = { ...newAssets[assetIdx].sensors[sensorIdx], currentValue: lastVal, history: item.readings };
        } else {
          newAssets[assetIdx].sensors.push({ id: sensorUniqueId, type: SensorType.VIBRATION, label: `${pointTag} (${item.unit})`, unit: item.unit, currentValue: lastVal, thresholdMin: 0, thresholdMax: item.unit === 'G' ? 3.0 : 4.5, history: item.readings, dataSource: 'periodic' });
        }
        const primarySensors = newAssets[assetIdx].sensors.filter(s => s.type === SensorType.VIBRATION);
        ...
        newAssets[assetIdx].severity = maxSeverity;
        newAssets[assetIdx].healthScore = getHealthScore(maxSeverity);
        newAssets[assetIdx].status = maxSeverity === 'D' ? 'critical' : maxSeverity === 'C' ? 'warning' : 'operational';
      });
      return newAssets;
    });
  ...
  const handleIncomingData = (assetId: string, sensorType: SensorType, value: number) => {
    setAssets(prevAssets => prevAssets.map(asset => {
      if (asset.id === assetId) {
        ...
        return { ...asset, sensors: updatedSensors, severity, status: severity === 'D' ? 'critical' : severity === 'C' ? 'warning' : 'operational', healthScore: getHealthScore(severity) };
      }
      return asset;
    }));
  };
  ...
  return (
    <div className="flex h-screen bg-[#f8fafc] text-slate-600 overflow-hidden font-light">
      ...
      {showAddAsset && <AddAssetModal onAdd={(a) => { setAssets([...assets, a]); setSelectedAssetId(a.id); }} onClose={() => setShowAddAsset(false)} />}
